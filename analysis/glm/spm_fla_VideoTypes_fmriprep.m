...
brainMask           = 'WholeBrain_fmriprep_WM_CSF_FWD_GS-noAROMA';         % whole brain or ROI
...
pipelineName        = 'fmriprep-21.0.1';                                                  % how is the folder named that contains preprocessed data
dataDir = fullfile (derivesDir, pipelineName);  % data that is used for the analysis
...
if smoothKernelSize == 0
    ...
    strcmp(smoothKernelSpace,'native')
else
...
numCon              = 6;
numSur              = 1;
numConfounds        = 8; % default number of realignment parameter, in model estimation overwritten but still 24
...
for s = 1:length(subNames)
    ...
    subDataDir          = fullfile(dataDir,         subNames{s}, 'func');
    ...
    % found
    if do.gunzip
        runs = cellstr(spm_select('FPList', subDataDir, 'space-MNI152NLin2009Asym_desc-smoothAROMAnonaggr_bold.nii.gz')); 
        matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.files = runs;
        matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.outdir = {subDataDir};
        matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.keep = true;
        spm('defaults', 'FMRI');
        spm_jobman('run', matlabbatch);
        clear matlabbatch
    end
    runs                = cellstr(spm_select('List', subDataDir, ['^s6' subNames{s} '.*space-MNI152NLin2009cAsym_desc-preproc_bold.nii$']));
    runs                = natsort(runs);
    numRuns             = length(runs); 
    ...
    if do.SpecifyDesign == 1 % Model specification
        ...
        confoundFiles = cellstr(spm_select('FPList',subDataDir,['^' subNames{s} '.*desc-confounds_timeseries.tsv']));
        confoundFiles = natsort(confoundFiles);
        ...
        for r = 1:numRuns % For each run
            ...
            dirfiles    = spm_select('ExtFPList',subDataDir, runs{r}, Inf);
            ...
            if fla.realignmentParametersFlag == 1
                ...
                % Parameters)
                confoundParameterNames = {
                    'csf', 'white_matter', 'framewise_displacement' ,'global_signal'
%                     'trans_x', 'trans_y', 'trans_z'...
%                     'rot_x', 'rot_y', 'rot_z'...
                    };
                numConfounds = length(confoundParameterNames);
                
                % Load alignment parameters
                [vals,heads, ~]     = tsvread(confoundFiles{r});
                confountTable       = array2table(vals(2:end,:));
                confountTable.Properties.VariableNames=heads;
                % replace NaNs with zeros
                confountTable = fillmissing(confountTable,'constant', 0);

                % convert table to timetable and perform high pass filter
                % on nuance regressors with same cutoff frequency as spm
                % performs on data (Hallquist et al 2013)
                TT = table2timetable(confountTable,'SampleRate', 1/(hdr{1}.RepetitionTime/1000));
                confoundTableFiltered = highpass(TT,1/128);
                % Add realignment parameters as regressors of no interest.
                for P = 1:length (confoundParameterNames)
                    matlabbatch{1}.spm.stats.fmri_spec.sess(r).regress(P).name = confoundParameterNames{P};
                    matlabbatch{1}.spm.stats.fmri_spec.sess(r).regress(P).val  = confoundTableFiltered.(confoundParameterNames{P});
                end
            end
        end
    ...
    if do.DefContrasts == 1
        ...
        C1 = repmat ([repmat([1     -1      0           0           zeros(1,numConfounds)], 1,2)       repmat([1               -1      0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C2 = C1*(-1);   
        C3 = repmat ([repmat([1     -1      0           0           zeros(1,numConfounds)], 1,2)       repmat([0               0       0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C4 = C3*(-1);
        C5 = repmat ([repmat([1      0      0           0           zeros(1,numConfounds)], 1,2)       repmat([-1              0       0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C6 = C5*(-1);
        C7 = repmat ([repmat([1      0     -1           0           zeros(1,numConfounds)], 1,2)       repmat([0               0       0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C8 = C7*(-1);
        C9 = repmat ([repmat([0     -1      1           0           zeros(1,numConfounds)], 1,2)       repmat([0               -1      1           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C10= C9*(-1);
        C11= repmat ([repmat([0      0      0           0           zeros(1,numConfounds)], 1,2)       repmat([1               -1      0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C12= C11*(-1);
        C13= repmat ([repmat([0      0      0           0           zeros(1,numConfounds)], 1,2)       repmat([1               0       -1          0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C14= C13*(-1);
        C15= repmat ([repmat([1     -1      0           0           zeros(1,numConfounds)], 1,2)       repmat([-1              1       0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);
        C16= C15*(-1);
        ...
        C17= repmat ([1     0       0           0           zeros(1,numConfounds)          -1      0       0           0           zeros(1,numConfounds) repmat([0       0       0           0           zeros(1,numConfounds)], 1,2)],1,numBlocks);
        ...
        C18= repmat ([0     0       0           0           zeros(1,numConfounds)      1       0       0           0           zeros(1,numConfounds)      -1      0       0           0           zeros(1,numConfounds)      0       0       0           0       	zeros(1,numConfounds)],1,numBlocks);
        ...
        C19= repmat ([repmat([0              1      0           0           zeros(1,numConfounds)], 1,2)       repmat([0              -1       0           0           zeros(1,numConfounds)], 1,2)], 1, numBlocks);

        %                       Magic No Magic    Surprise    Response    Realigment
        C20= repmat (repmat([   1     1           1           -3          zeros(1,numConfounds)],1,4),1,numBlocks);
        C21= C20*(-1);
